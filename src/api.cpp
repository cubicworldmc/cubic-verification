...
bool API::conn_socket() {
    ...
    if (inet_pton(AF_INET, hinfo.addr.c_str(), &s_addr.sin_addr) != 1) {
        std::cout << "invalid address: " << hinfo.addr << std::endl;
        close(m_socket);
        m_socket = -1;
        return false;
    }
...
Response API::send_request(const std::string& kind, const std::string& player,
    ...
    if (enc.size() < NONCE_LEN + TAG_LEN)
        return {0, "", "", "", "enc size is small", false};
    ...
    try {
        ...
        disconnect();
        return {0, "", "", "", "failed to decrypt", false};
    }
